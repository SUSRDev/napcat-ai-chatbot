...
export async function captureAndRegisterEmoji(opts) {
  ...
  if (typeof callVision === 'function') {
    const dataUrl = `data:image/${ext.slice(1) === 'jpg' ? 'jpeg' : ext.slice(1)};base64,${buf.toString('base64')}`;
    const raw = await callVision({ systemPrompt: promptTemplate, userText: '请分析这张图片。', imageUrls: [dataUrl] });
    vlm = parseEmojiVlmJson(raw);
  }
  ...
  setEmojiStatus(rec, autoRegister ? 'owned' : (vlm ? 'pending' : 'unknown'));

  if (autoRegister) store.emojiCacheStats.registered = (Number(store.emojiCacheStats.registered) || 0) + 1;
  return { status: autoRegister ? 'registered' : 'cached', emoji: rec };
}
