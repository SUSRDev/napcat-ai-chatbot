...
import { buildShellTools, executeShellCommand } from './agent-shell.mjs';
...
export async function executeBuiltinTool(toolDef, args, runtime) {
  ...
  if (kind === 'shell_exec') {
    const { cfg } = runtime;
    return executeShellCommand(cfg, args);
  }
  if (kind === 'browser_snapshot' || kind === 'browser_act') {
...
export async function runAgentToolLoop(params) {
  ...
  const mcpTools = cfg.mcpEnabled && mcpHub ? mcpHub.getOpenAiTools() : [];
