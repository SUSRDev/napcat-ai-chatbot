...
async function downloadFile(url, destPath, onProgress, fileName, mirrorLabel) {
  const name = fileName || path.basename(destPath);
  const via = mirrorLabel ? `（${mirrorLabel}）` : '';
  report(onProgress, 'download', `正在连接下载 ${name}${via}…`, 12);
...
async function extractZip(zipPath, destDir, onProgress) {
  ...
  const tryUnzip = () => execFileAsync('unzip', ['-o', zipPath, '-d', destDir], { timeout: 180000 });
  const tryPython = (bin) => execFileAsync(bin, ['-m', 'zipfile', '-e', zipPath, destDir], { timeout: 180000 });
  try {
    ...
    try {
      report(onProgress, 'extract', `正在解压 ${zipName}（python3）…`, 58);
      await tryPython('python3');
      report(onProgress, 'extract', '解压完成', 64);
    } catch {
      try {
        report(onProgress, 'extract', `正在解压 ${zipName}（python）…`, 58);
        await tryPython('python');
        report(onProgress, 'extract', '解压完成', 64);
      } catch {
        throw new Error('解压失败：请安装 unzip 或 python3，Windows 环境请使用 PowerShell');
      }
    }
  }
}
...
export async function applyReleaseUpdate(pluginDir, releaseInfo, logger, onProgress, mirrorOptions) {
  ...
  let mirrorName = 'GitHub 原始';
  if (mirrorOptions?.resolveDownloadUrl) {
    const resolved = await mirrorOptions.resolveDownloadUrl(releaseInfo.downloadUrl, onProgress);
    downloadUrl = resolved.url;
    mirrorName = resolved.mirrorName || mirrorName;
  } else if (mirrorOptions?.url) {
    downloadUrl = mirrorOptions.url;
    mirrorName = mirrorOptions.mirrorName || mirrorName;
  }
  ...
  try {
    await downloadFile(downloadUrl, zipPath, onProgress, assetName, mirrorName);
