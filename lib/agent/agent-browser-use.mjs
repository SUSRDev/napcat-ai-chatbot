...
import { runCommand, runCommandInDir, runNodeExe } from './process-run.mjs';
import { getAgentRuntimeDir, getPlaywrightBrowsersDir, humanizeProcessError } from '../skillhub/skillhub-cli.mjs';

export const BROWSER_USE_REV = '1.0.3';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const RUNNER_SRC = path.join(__dirname, '..', 'scripts', 'browser', 'browser_use_runner.py');
...
function venvScriptsDir(venvDir) {
  return process.platform === 'win32'
    ? path.join(venvDir, 'Scripts')
    : path.join(venvDir, 'bin');
}

function venvPythonEnv(venvDir, extra = {}) {
  const scripts = venvScriptsDir(venvDir);
  return {
    ...process.env,
    VIRTUAL_ENV: venvDir,
...
/** Windows 下 venv python 走 execFile，避免 cmd 破坏引号 */
async function runVenvPython(pyExe, venvDir, args, opts = {}) {
  const env = venvPythonEnv(venvDir, opts.env);
  const runOpts = { ...opts, env };
  if (process.platform === 'win32') {
    return runNodeExe(pyExe, args, runOpts);
  }
  return runCommand(pyExe, args, runOpts);
}

function isPlaywrightInstalledInVenv(pluginRoot) {
  const venvDir = getBrowserUseVenvDir(pluginRoot);
  const winSite = path.join(venvDir, 'Lib', 'site-packages', 'playwright');
  if (fs.existsSync(winSite)) return true;
  try {
    const libDir = path.join(venvDir, 'lib');
    if (fs.existsSync(libDir)) {
      for (const name of fs.readdirSync(libDir)) {
        if (name.startsWith('python') && fs.existsSync(path.join(libDir, name, 'site-packages', 'playwright'))) {
          return true;
        }
      }
    }
...
export function hasBrowserUseChromium(pluginRoot) {
  try {
    const browsersDir = getPlaywrightBrowsersDir(pluginRoot);
    return fs.existsSync(browsersDir) && fs.readdirSync(browsersDir).some((n) => /chromium/i.test(n));
  } catch {
    return false;
  }
}

async function pipInstall(pyExe, packages, mirror, pipEnv, onLine, timeoutMs = 900000) {
  const pipArgs = ['install', ...packages, '--no-warn-script-location'];
  if (mirror?.url) pipArgs.push('-i', mirror.url);
  return runCommand(pyExe, ['-m', 'pip', ...pipArgs], {
    timeoutMs,
    env: pipEnv,
    onLine: (l) => onLine?.(`  ${l}`)
  });
...
function getVerifyImportScriptPath(pluginRoot) {
  const dir = path.join(getAgentRuntimeDir(pluginRoot), 'browser-use');
  fs.mkdirSync(dir, { recursive: true });
  const scriptPath = path.join(dir, '_verify_import.py');
  fs.writeFileSync(scriptPath, 'import browser_use\nprint("ok")\n', 'utf8');
  return scriptPath;
}

/** Windows 下不经 cmd.exe，直接 execFile 执行 venv python + 脚本文件 */
async function verifyBrowserUseImport(pluginRoot, onLine) {
  const venvDir = getBrowserUseVenvDir(pluginRoot);
  const py = venvPython(venvDir);
  if (!fs.existsSync(py)) return { ok: false, error: 'venv Python 不存在' };
  const scriptPath = getVerifyImportScriptPath(pluginRoot);
  onLine?.(`  校验脚本: ${scriptPath}`);
  const r = await runVenvPython(py, venvDir, [scriptPath], { timeoutMs: 120000 });
  const out = `${r.stdout || ''}\n${r.stderr || ''}`;
  const ok = r.ok && /\bok\b/.test(r.stdout || '');
  if (!ok && out.trim()) onLine?.(`  校验失败: ${out.trim().split(/\r?\n/).slice(0, 5).join(' | ')}`);
  return { ok, error: ok ? '' : (humanizeProcessError(out) || 'import browser_use 失败') };
}

/** 仅校验导入与 Chromium，不重新 pip 安装 */
export async function verifyAgentBrowserUseEnv(pluginRoot, onLine) {
  if (!isBrowserUseInstalled(pluginRoot)) {
    return { ok: false, importOk: false, hasChromium: false, error: 'browser-use 包未安装' };
  }
  const importResult = await verifyBrowserUseImport(pluginRoot, onLine);
  const hasChromium = hasBrowserUseChromium(pluginRoot);
  const ok = importResult.ok && hasChromium;
  let error = '';
  if (!importResult.ok) error = importResult.error || 'import browser_use 失败';
  else if (!hasChromium) error = 'Chromium 未就绪';
  return { ok, importOk: importResult.ok, hasChromium, error };
}

export async function getBrowserUseEnvStatus(pluginRoot, cfg = {}) {
  const py = await detectPython();
  const venvDir = getBrowserUseVenvDir(pluginRoot);
  const packageInstalled = isBrowserUseInstalled(pluginRoot);
  const verify = packageInstalled ? await verifyBrowserUseImport(pluginRoot) : { ok: false, error: '' };
  const hasChromium = hasBrowserUseChromium(pluginRoot);
  const importOk = !!verify.ok;
  const ready = importOk && hasChromium;
  let statusText = '未安装';
  if (ready) statusText = 'ready';
  else if (importOk && !hasChromium) statusText = 'need_chromium';
  else if (packageInstalled) statusText = 'need_verify';
  else if (py.ok) statusText = 'need_install';
  else statusText = 'need_python';
  ...
  return {
    ...
    installed: ready,
    statusText,
    importError: verify.error || '',
    envReady: !!cfg.agentBrowserUseEnvReady && ready,
...
export function armBrowserUseInstall(mirror = '') {
  if (setupState.running) return false;
  setupState = {
    running: true,
    ok: false,
    step: 'init',
...
export async function installAgentBrowserUse(pluginRoot, opts = {}) {
  if (!setupState.running) {
    if (!armBrowserUseInstall(opts.mirror || 'auto')) {
  ...
  try {
    onLine(`=== browser-use 环境安装开始 (rev ${BROWSER_USE_REV}) ===`);
    ...
    const alreadyPkg = isBrowserUseInstalled(pluginRoot);
    if (alreadyPkg) {
      onLine('  检测到 browser-use 包已存在，跳过重复安装');
      onProgress(55, 'browser-use 包已存在', 'install');
    } else {
      const inst = await pipInstall(pyExe, ['browser-use[core]'], mirror, pipEnv, onLine);
      if (!inst.ok) {
        throw new Error(humanizeProcessError(inst.stderr || inst.stdout) || 'pip install browser-use 失败');
      }
    }

    onProgress(58, '安装 Python Playwright 驱动…', 'pip');
    if (isPlaywrightInstalledInVenv(pluginRoot)) {
      onLine('  检测到 playwright 包已存在，跳过重复安装');
      onProgress(65, 'playwright 包已存在', 'pip');
    } else {
      const pwPkg = await pipInstall(pyExe, ['playwright'], mirror, pipEnv, onLine);
      if (!pwPkg.ok) {
        throw new Error(humanizeProcessError(pwPkg.stderr || pwPkg.stdout) || 'pip install playwright 失败');
      }
    }

    onProgress(72, '下载 Chromium 浏览器…', 'chromium');
    let chromiumOk = hasBrowserUseChromium(pluginRoot);
    if (chromiumOk) {
      onLine('  Chromium 已就绪，跳过下载');
    } else {
      const chromiumBaseEnv = { ...pipEnv, PLAYWRIGHT_BROWSERS_PATH: browsersDir };
      const pw = await runVenvPython(pyExe, venvDir, ['-m', 'playwright', 'install', 'chromium'], {
        timeoutMs: 900000,
        env: chromiumBaseEnv,
        onLine: (l) => onLine(`  ${l}`)
      });
      ...
      if (!pw.ok) {
        onLine('  官方源失败，尝试国内镜像…');
        const chromiumMirrorEnv = {
          ...pipEnv,
          PLAYWRIGHT_BROWSERS_PATH: browsersDir,
          PLAYWRIGHT_DOWNLOAD_HOST: process.env.PLAYWRIGHT_DOWNLOAD_HOST || 'https://npmmirror.com/mirrors/playwright'
        };
        const pw2 = await runVenvPython(pyExe, venvDir, ['-m', 'playwright', 'install', 'chromium'], {
          timeoutMs: 900000,
          env: chromiumMirrorEnv,
          onLine: (l) => onLine(`  ${l}`)
        });
        chromiumOk = pw2.ok || hasBrowserUseChromium(pluginRoot);
        if (!chromiumOk) {
          onLine('  Chromium 下载失败，可点击「开始部署」重试');
        }
      }
    }

    onProgress(90, '验证 browser-use 导入…', 'verify');
    onLine('  验证 browser-use 导入…');
    const verify = await verifyBrowserUseImport(pluginRoot, onLine);
    if (!verify.ok) {
      throw new Error(verify.error || 'browser-use 安装后导入失败，请查看日志');
    }

    ensureRunnerScript(pluginRoot);
    if (!chromiumOk && !hasBrowserUseChromium(pluginRoot)) {
      onProgress(95, 'browser-use 包已就绪，Chromium 待安装', 'verify');
      onLine('[提示] browser-use 已可导入，但 Chromium 未就绪。请再次点击部署以下载浏览器。');
      setupState.ok = false;
      setupState.step = 'need_chromium';
      setupState.error = 'Chromium 未安装完成';
      return { ok: false, partial: true, needChromium: true, venvDir, mirror: mirror.id };
    }
...
export async function runBrowserUseTask(pluginRoot, cfg, params) {
  const status = await getBrowserUseEnvStatus(pluginRoot, cfg);
  if (!status.importOk) {
    return '错误：browser-use 环境未就绪，请在 Agent 设置中选择 browser-use 并点击「一键部署环境」。';
  }
  if (!status.hasChromium) {
    return '错误：browser-use 包已安装，但 Chromium 浏览器未就绪。请再次点击「一键部署 browser-use」完成浏览器下载。';
  }
