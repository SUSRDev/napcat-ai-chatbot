...
async function fetchPageText(url) {
  const res = await fetch(url, {
    headers: { 'User-Agent': 'napcat-plugin-chat-bot/2.6 AgentBrowser' },
 ...
 * @param {string} url
 */
async function playwrightSnapshot(pluginRoot, url) {
  ...
  try {
    ...
    return { url, title, text, screenshot: shotPath };
  } finally {
    await browser.close();
  }
}
 ...
 */
async function playwrightAction(pluginRoot, params) {
  const pw = await loadPlaywright(pluginRoot);
...
export async function executeBrowserTool(cfg, pluginRoot, toolKind, params) {
  ...
  if (toolKind === 'browser_snapshot') {
    ...
    if (cfg.agentBrowserUsePlaywright !== false) {
      const pw = await playwrightSnapshot(pluginRoot, url);
      if (pw) {
        return `标题: ${pw.title}\n截图: ${pw.screenshot}\n\n正文:\n${pw.text}`;
      }
    }
    try {
      const f = await fetchPageText(url);
  ...
  if (toolKind === 'browser_act') {
    return playwrightAction(pluginRoot, params);
  }
