...
/** @param {object} store @param {Record<string, unknown>} cfg @param {string} groupId @param {string} recentContext @param {Function} llmText @param {object} [db] */
export async function buildHeuristicMemoryBlock(store, cfg, groupId, recentContext, llmText, db = null) {
...
export async function runMaisakaFakeHumanChat(opts) {
  ...
  const memoryBlock = await buildHeuristicMemoryBlock(store, cfg, groupId, recentContext, llmText, db);
  ...
  if (usePlannerLoop && typeof llmWithTools === 'function') {
    ...
    if (outbound.length || loop.actions.some((a) => a.tool === 'no_action')) {
      return { action: outbound.length ? 'multi' : 'skip', outbound, darkMode, plan: loop, reasoning: loop.reasoning };
    }
  }
  ...
  const replySystem = buildReplySystemPrompt({
    ...
    replyerOutputInstruction: `可输出 1-${Math.min(4, Math.max(1, Number(cfg.fakeHumanBurstMaxMessages) || 4))} 条极短口语句，用 ||| 分隔（每条不超过 ${Math.max(8, Math.min(40, Number(cfg.fakeHumanBurstMaxLen) || 20))} 字）。不要 markdown，不要 JSON。`
  });
  const replyUser = [
    '【回复信息参考】',
