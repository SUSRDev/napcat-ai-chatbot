...
export const DEFAULT_STICKER_SELECTION_PROMPT = `你需要根据上下文和当前语气，从候选表情包中选出最贴合场景的一张。
其中包含一张 {grid_rows}x{grid_columns} 的表情包拼图，一共 {emoji_count} 个位置。
每张小图左上角都有一个较大的序号，范围是 1 到 {emoji_count}。
候选表情均已根据描述/标签预筛选，请从中选出与当前对话氛围、情绪最匹配的一张。
若都不合适，仍选相对最不违和的一张，并说明理由。
...
只有在完全无关、刷屏或不宜插话时才用 no_action；多数情况下应至少 reply 一句或 send_qq_face。

最近群消息：
{message_window}`;

export const DEFAULT_FAKEHUMAN_ACTION_CHOOSE_PROMPT = `根据最近群聊内容，选一种互动方式。只输出一个数字：
1=发文字回复（会走 Planner 生成内容，可含表情）
2=只@对方
3=只戳一戳对方
不要其他文字。`;
