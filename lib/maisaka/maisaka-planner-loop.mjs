...
export async function runMaisakaPlannerLoop(opts) {
  ...
  for (let r = 0; r < maxRounds; r++) {
    ...
    // 模型常把正文写在 content，却只调 send_qq_face/send_emoji → 补一条 reply，避免只发表情
    const roundHasReply = toolCalls.some((tc) => (tc.function?.name || tc.name) === 'reply');
    const plainFromContent = content.replace(/\[CQ:[^\]]+\]/gi, '').replace(/\s+/g, ' ').trim();
    if (plainFromContent && !roundHasReply) {
      const roundStartIdx = actions.length - toolCalls.length;
      actions.splice(roundStartIdx, 0, {
        tool: 'reply',
        args: { message: plainFromContent.slice(0, maxLen) },
        result: 'content_with_tools'
      });
      // 同轮误调 send_qq_face/send_emoji 时优先发文字，去掉重复表情
      for (let i = actions.length - 1; i >= roundStartIdx + 1; i--) {
        if (actions[i].tool === 'send_qq_face' || actions[i].tool === 'send_emoji') {
          actions.splice(i, 1);
        }
      }
    }
