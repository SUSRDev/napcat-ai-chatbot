 ...
 * 智能搜索：国内 / 国外多平台并行检索
 */
import { buildCookieHeaderForUrl } from '../agent/agent-cookies.mjs';
...
/** B站公开搜索 API（免 Key；有 Cookie 时自动附加） */
export async function bilibiliSearch(query, cfg = {}) {
  ...
  try {
    ...
    const cookieHeader = buildCookieHeaderForUrl(cfg, url);
...
export async function smartSearchMulti(query, cfg, runners, regionOverride = null) {
  ...
  if (region === 'domestic') {
    tasks.push({ source: '哔哩哔哩', run: () => bilibiliSearch(q, cfg) });
