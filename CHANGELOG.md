...
- **群黑话候选词过滤**：大幅扩充日常口语停用词表，新增前缀/后缀/句子结构规则，减少无意义词条入库，候选词上限从 10 条收紧至 6 条。

- **黑话推断参数可配置**：新增 `slangInferMinCount`（触发推断的最低出现次数，默认 1）和 `slangInferBatchLimit`（单次批量上限，默认 12）两个配置项，替换此前的硬编码值。

- **推断触发条件放宽**：消息触发实时推断时不再要求出现次数 ≥ 2，改为读取 `slangInferMinCount` 配置，已被拒绝的词条不再重复推断。

### 新增

- **黑话规则清理接口**：新增 `/slangs/sanitize` API 端点，可一键清理存储中的无效/冲突黑话条目并持久化。

- **`sanitizeObservedSlangs` 自动清理**：批量推断前自动执行一次规则清理，返回结果中包含 `sanitized` 清理计数。
...
- **颜文字自定义词库**：新增 `kaomojiExtraList` 配置项，支持最多 30 个自定义颜文字，与内置词库合并参与随机选取。
- **颜文字触发概率控制**：新增 `kaomojiUseChance` 配置项（默认 `0.35`），可通过 API 动态调整颜文字出现频率。

### 改进

- **颜文字作用域分离**：`buildKaomojiHint` 新增 `scope` 参数，区分 `chat` 和 `fakehuman` 两种模式，分别读取各自的开关配置，不再共用同一判断逻辑。
- **Agent 搜索词兜底补全**：`web_search` 工具在模型未传 `query` 参数时，自动从用户原始消息提取搜索意图，支持中文语义模式匹配（如"X 是什么""查一下 X"），并记录 `info` 日志说明补全来源。
- **工具调用参数解析增强**：`parseToolCallArgs` 新增对模型偶发返回非 JSON 字符串的容错处理，将合法短字符串直接作为 `query` 使用，避免工具调用静默失败。
- **运行时上下文透传**：Agent 工具循环中新增 `userText` 和 `log` 字段透传至工具执行层，搜索、颜文字等工具可直接访问当前用户消息和日志函数。
...
- **颜文字库扩充**：`开心` 场景新增 `AWA`、`awa`、`:D` 等表情，`难过` 场景新增 `qwq`、`QAQ`、`:(` 等，情绪表达更贴近年轻用户习惯。
- **惊讶触发修复**：修正 `pickKaomojiForContext` 中问号匹配的正则表达式，改为 `[?？]` 以同时兼容半角与全角问号，避免原有无效字符导致匹配失败。
